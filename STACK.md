...
| UI | **Tailwind v4 + shadcn/ui, preset oficial** (estilo nova, base neutral, dark) | Tema pronto para formulário, tabela e diálogo. Dois toques de marca: IBM Plex e o creme no `--primary` |
| Visualização | **Camada própria em SVG, monocromática** *(decidido em 06/08, revendo "sem design próprio")* | O admin virou painel de decisão, e gráfico é o meio. Uma hue só — o creme em rampa ordinal validada contra a superfície escura — porque o dado do estúdio é magnitude, não identidade; paleta categórica colorida enterraria o que importa. Status (verde/âmbar/vermelho) fica reservado a estado. Números em IBM Plex **Mono**: a ficha técnica é o ativo do Tino, e o painel tem a mesma cara. Sem biblioteca de gráfico — SVG direto, sem dependência nova. Componentes em `src/components/viz` |
